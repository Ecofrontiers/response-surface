...
export default function ENSPanel({ agents, onClose }: ENSPanelProps) {
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="h-1 bg-gradient-to-r from-[var(--status-standby)] via-[var(--viz-3)] to-[var(--status-standby)]" />
        <div className="sticky top-0 bg-[var(--color-header)] border-b border-[var(--border-default)] px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-[var(--color-text)]">ENS Identity Registry</h2>
        ...
        <div className="px-6 py-5 space-y-6">
          <div>
            <h3 className="text-[10px] uppercase tracking-wider text-[var(--color-text-placeholder)] mb-3">Text Record Schema</h3>
            <div className="grid grid-cols-3 gap-1.5">
              {TEXT_RECORD_SCHEMA.map((r, i) => (
                ...
                >
                  <span className="font-[var(--font-mono)] text-[var(--color-text-placeholder)]">{r.key}</span>
                  <div className="text-[var(--color-text-placeholder)] mt-0.5">{r.label}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {agents.map((agent, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="rounded-xl border bg-[var(--color-header)] overflow-hidden"
                    ...
                    <div className="px-4 py-3 flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-[var(--color-text)]">{agent.ensName}</span>
                          <span
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-[9px] text-[var(--color-text-placeholder)]">/1000</div>
                      </div>
                    </div>
...
function TextRecordRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-[var(--color-base)]">
      <span className="text-[9px] text-[var(--color-text-placeholder)] font-[var(--font-mono)] shrink-0">{label}</span>
      <span className={`text-[9px] font-[var(--font-mono)] truncate ${highlight ? 'text-cyan-400' : 'text-[var(--color-text-placeholder)]'}`}>
